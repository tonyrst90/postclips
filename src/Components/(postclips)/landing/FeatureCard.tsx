...
const FeatureCard = ({ gradient, children, id }: FeatureCardProps) => {
    ...
    return (
        <div
            className={`absolute inset-0 h-full w-full rounded-2xl transition-opacity ${inViewFeature === id
...
export const Test = ({ id }: CardProps) => {
    return (
...
export const Music = ({ id }: CardProps) => {
    ...
    return (
        <FeatureCard id={id} gradient="from-[#f7fff5] to-[#adffd8]">
            <img
                className={`absolute left-[10%] top-[10%] w-[20%] rounded-xl shadow-lg transition-transform ${isFullscreen ? "scale-0" : "scale-100"
                    }`}
                src="https://www.frontend.fyi/playground-assets/amie/song-1.webp"
            />
            <img
                className={`absolute left-[70%] top-[20%] w-[25%] rounded-xl shadow-lg transition-transform ${isFullscreen ? "scale-0" : "scale-100"
                    }`}
                src="https://www.frontend.fyi/playground-assets/amie/song-2.webp"
            />
            <img
                className={`absolute left-[20%] top-[60%] w-[30%] rounded-xl shadow-lg transition-transform ${isFullscreen ? "scale-0" : "scale-100"
                    }`}
                src="https://www.frontend.fyi/playground-assets/amie/song-3.webp"
            />
